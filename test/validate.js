...
// tactics-puzzles.js is generated by test/build-tactics-puzzles.js from the
// diagnostic scan's cache and may not exist yet (e.g. a fresh clone, or
// before the first scan) -- tolerate its absence rather than failing.
...
if (fs.existsSync(tacticsPath)) {
  ...
  tacticsPuzzles = extractTactics();
}

var ALL = data.P.concat(data.H).concat(tacticsPuzzles);

console.log('Validating ' + ALL.length + ' lines (' + data.P.length + ' Ponziani + ' + data.H.length + ' Hippo + ' + tacticsPuzzles.length + ' Tactics) parsed from index.html' + (tacticsPuzzles.length ? ' + tactics-puzzles.js' : '') + '\n');
